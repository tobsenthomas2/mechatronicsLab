...
class encoder:
    def __init__ (self,CH1,CH2):
        ...
        #sets up channel 1&2 for encoder A&B
        timer_ch1 = timer.channel(1,pyb.Timer.ENC_A)
        timer_ch2 = timer.channel(2,pyb.Timer.ENC_B)
        
        #set pins for PB6/7
        pinPB6 = pyb.Pin (pyb.Pin.board.PB6, pyb.Pin.OUT_PP)
        pinPB7 = pyb.Pin (pyb.Pin.board.PB7, pyb.Pin.OUT_PP)
        
        """!Reconnect USB and write code that sets up a timer and
its CH1 and CH2 channels in encoder counting mode.
Only one timer will work with those pins;"""
        self.CH1 = 
